...
| QA run | QA protocol, current QA regression doc, current phase plan | Historical/resolved bugs only when checking recurrence |
| Code review | Review protocol, review checklist, current diff, relevant phase acceptance criteria | Full PRD set only for boundary questions |
| Deployment/ops issue | README, latest tech plan section 6, affected config/deploy docs | PRD details |
| Harness/workflow improvement | This file, root `AGENTS.md`, `docs/harness-engineering.md`, workflow docs | Business implementation docs |
